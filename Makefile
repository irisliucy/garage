...
# BuildKit is required for the --secret flag in docker. We use it to mount mjkey.txt during build time
export DOCKER_BUILDKIT=1

build-test: TAG ?= rlworkgroup/garage-test-18.04
build-test: docker/Dockerfile
	docker build \
		-f docker/Dockerfile \
		--target garage-test-18.04 \
		--secret id=mjkey,src=$(MJKEY_PATH) \
		-t rlworkgroup/garage-test-18.04 \
		${BUILD_ARGS} \
		.

test:  ## Run the garage-test-18.04 docker target
test: RUN_ARGS = --memory 7500m --memory-swap 7500m
test: CONTAINER_NAME ?= garage-test
test: build-test
...
run-headless: CONTAINER_NAME ?= garage-nvidia
